...
% ---------------------------------------------------------------- fig 3 --
    function fig3()
        for cse = {'E-AC-02b', 'E-AC-01a'}
            id = cse{1}; f = newfig(1000, 560); tl = tiledlayout(f, 2, 4, 'TileSpacing', 'compact'); any_ = false;
            for i = 1:6
                X = ts(id, V{i}); if isempty(X), continue; end, any_ = true;
                [ti, ia] = load_iac(tdir, id, V{i});
                ax = nexttile(tl, i + (i > 3)); hold(ax, 'on');   % leave tiles 4, 8 for bars
...
% ---------------------------------------------------------------- fig 4 --
    function fig4()
        ...
        for i = 1:6, b(i).FaceColor = col(i, :); b(i).DisplayName = V{i}; end
        hold(ax, 'on');
        for a = 1:numel(ids), for i = 1:6, if Tr(a, i) > 0
...
% ---------------------------------------------------------------- fig 5 --
    function fig5()
        ...
        if ~isempty(sc)
            for i = 1:6, for id = {'E-BAT-01b', 'E-BAT-01n'}
        ...
        if ~isempty(sc)
            for i = 1:6, for id = {'E-BAT-02b', 'E-BAT-02c'}
                r = sc(sc.test_id == id{1} & sc.VARIANT_NAME == V{i}, :);
                if ~isempty(r) && ismember('P_charge_dur_kW', r.Properties.VariableNames), plot(ax, r.amp, r.P_charge_dur_kW, 'o', 'Color', col(i, :)); end
            end, end
        end
...
% ---------------------------------------------------------------- fig 6 --
    function fig6()
        X0 = []; for i = 1:6, X0 = ts('E-BAT-02b', V{i}); if ~isempty(X0), break; end, end
        ...
        for i = 1:6, X = ts('E-BAT-02b', V{i}); if isempty(X), continue; end, plot(ax, X.t, X.Vdc_real, 'Color', col(i, :), 'DisplayName', V{i}); end
...
% ---------------------------------------------------------------- fig 7 --
    function fig7()
        ...
        for k = 1:3
            ...
            xticks(ax, 1:nI); xticklabels(ax, ids); yticks(ax, 1:6); yticklabels(ax, V); set(ax, 'TickLabelInterpreter', 'none');
            for a = 1:nI, for i = 1:6, if ~isnan(Hk(i, a))
