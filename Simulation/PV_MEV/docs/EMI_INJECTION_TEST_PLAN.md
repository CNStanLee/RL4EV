...
### 11.1 补充：HGQ2 谐波估计变种（2026-09-04，104 次运行，8 种策略）

阶段二把丢失的 HGQ2 Residual-BLS 一周期谐波估计模型接回（`build_estimator.m`），新增两种策略：MPCC_D_M1（原始 HGQ2 估计）与 MPCC_D_H1（FFT1 + HGQ2 融合，estimation_src 6）。全矩阵在加入 EMI 检测器和估计器子系统后的模型上重跑，原 6 种策略的 78 行 scorecard 与本节上文逐值相同，图 2 到图 8 与 §9.12 波形图集已加入两列。

| 用例 | MPCC_D | MPCC_D_F1 | MPCC_D_F10 | MPCC_D_R | MPCC_D_M1 | MPCC_D_H1 |
|---|---:|---:|---:|---:|---:|---:|
| 基线 THD50 | 5.48% | 2.76% | 2.70% | 2.80% | 3.18% | 3.02% |
| E-AC-01a THD50 注入中 | 6.19% | 4.29% | 4.29% | 4.72% | 4.12% | 4.08% |
| E-AC-01b THD50 注入中 | 8.57% | 7.78% | 7.80% | 8.19% | 5.97% | 6.42% |
| E-BAT-01b THD50 注入中 | 5.95% | 3.00% | 3.05% | 2.99% | 3.51% | 3.34% |
| E-DC-01b THD50 注入中 | 6.93% | 3.71% | 3.69% | 3.68% | 4.26% | 4.08% |
| E-DC-01b / 01c 恢复时间 | 120 / 122 ms | 180 / 200 ms | 120 / 160 ms | 120 / 140 ms | 180 / 260 ms | 180 / 200 ms |
| 新增保护动作 | — | E-BAT-02c OC | — | E-BAT-02c、E-DC-01c、E-MUL-01 OC | E-DC-01c OC | E-DC-01c OC |

HGQ2 变种基线略差（8-bit 量化、旧工况训练），交流链偏置下略优（不依赖被污染的锁相谐波基准），直流 / 电池链偏置下差约 0.5 pp，恢复时间与 F1 同级。检测器（阶段二）在同一战役里对 12 个单通道用例 × 8 策略全部检出，延迟 2 周期，注入前零误报；双通道 E-MUL-01 只检出 Vdc 分量，Iac +5 A 分量漏检。数据与报告：`EMI_DET_FPGA/README.md` 评估战役一节。
