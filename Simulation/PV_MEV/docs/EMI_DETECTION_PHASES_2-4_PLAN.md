...
| 日期 | 事项 | 结果 |
|---|---|---|
| 2026-09-03 | 测试台扩展：噪声槽位、充电电流参考阶跃与 Vref 阶跃两种良性瞬态、CV 段工作点、`run_injection('dataset')` | 模型结构变化使 12 份快照全部重做（约 1.5 h）；D0001 试跑通过 |
| 2026-09-03 | `EMI_DET_FPGA/`：41 个逐周期特征、数据集组装、随机森林检查、HGQ2 MLP 训练脚本；conda 环境 `hgq2` | 阶段一 84 条运行、留出一种策略：随机森林 95 到 100%，浮点 MLP 99.3%，HGQ2 MLP（激活 4/7、权重 2/7）98.7%，延迟中位 1 周期；`alkaid` 未装（不在 PyPI） |
| 2026-09-03 04:36 | 随机数据集 D0002 到 D0240 启动，三进程 | 每次约 18 min（撤除后再记 0.3 s），约 9 次/h，预计 09-04 07:00 完成；发现并修正了晚注入用例的空窗口错误 |
| 2026-09-03 10:20 | 60 条随机运行的中期评估 | 按运行留出：随机森林在 1% 误报预算下检出 29 / 51 条注入运行（Vdc 3/11、Vac 8/13、Iac 7/12、Vbat 13/17、Ibat 4/10），漏检集中在小幅值（Ibat < 4 A、Vbat < 8 V）和 Vdc 链的三角 / 正弦形状（外环滤掉了）；单条留出时 +70 V 阶跃可检出，+114 V 概率 0.3 到 0.4，说明是正样本运行数太少（每通道约 10 条）而不是特征不可见。HGQ2 MLP 在 60 条上过拟合（训练 0.95，测试 0.69）。对策：加入基线相对特征与策略 one-hot（暂无增益）、两个物理交叉校验特征（D_dcdc·Vdc_int/Vbat_int − 1，Vbat_int − D_dcdc·Vdc_int）、逐通道多标签输出头、按误报预算取阈值；Iac 正弦 / Hall 两类在随机采样中占比不到 4%，`run_injection('dataset',…,struct('focus','iac'))` 补充模式已加，待主数据集结束后补 60 条 |
| 2026-09-04 02:40 | 随机数据集 240 条全部完成（0 失败，约 22 h） | 组成：无注入 55、单通道 146、双通道 39；CC 185 / CV 55；良性瞬态：充电电流阶跃 78、Vref 阶跃 47、测量噪声 52。逐周期样本 8709（稳态无注入 6425）。Iac 正弦 5 条、Hall 3 条，仍偏少 |
...
| 2026-09-04 06:20–09:00 | det_Iref 改接 Speed Regulator2 后快照重做，评估战役重启（07:00，3 进程，预计 17:30 完成）；上板组件与 ONNX 位精确修正 | （1）前 3 次运行的 SIL 对照：43 个特征中除占空比两项外与 Python 参考一致到 1e-13，ONNX 输出一致到 5e-14；CRPR 的 iac_ref_corr 差异只在第一个周期（日志首样本 NaN），不是错误。占空比两项 `d_mean`、`d_ff_mismatch` 与离线特征相差最大 5e-3（占空比以 20 kHz 变化、10 kHz 采样相位不同所致，标准化后约 0.05 sigma），其余特征一致到 1e-13；曾误判为『滞后一个周期』，实为 Python 按周期起点、Simulink 按周期终点标时的比较错误，经 MATLAB 追溯 `det_D` 与日志 `D` 同取自 `Saturation` 输出，对齐后无滞后。（2）上板：`scripts/make_board_components.py` 生成 `HLS_PRJ/emi_detector`、`HLS_PRJ/harmonic_estimator` 两个 Vitis 统一流程组件（与 `HLS_PRJ/mpcc` 同款 `hls_config.cfg` / `vitis-comp.json` / s_axilite），包装层分别负责标准化+阈值、CycleNorm+解码；本机无 Vitis，用 zig c++ 做 C 仿真：估计器与 ONNX 参考逐值相同，检测器 max |Δlogit| 4.6e-5、1089 周期标志字全部一致。（3）发现 Keras 导出的 detector.onnx 用四舍六入五成双而硬件逢五进一，8709 周期 max |Δlogit| 4.3、68 个周期判定不同；`export_bitexact_onnx.py` 生成的 `detector_bitexact.onnx` 与 Keras 逐值相同（max 0），战役结束后替换并用 `redecide.py` 复核（前 6 次运行复核：判定无差异）。 |
| 2026-09-04 16:45–17:30 | 评估战役完成（104 / 104 OK，07:00 到 16:45），后处理与汇报 | （1）resummarize + merge：原 6 种策略 78 行与阶段一逐值相同（检测器、估计器对它们是纯观测）；scorecard 新增 MPCC_D_M1、MPCC_D_H1 各 13 行。（2）SIL 检测器：96 / 104 检出（12 个单通道用例 × 8 策略全部检出；E-MUL-01 的 Iac +5 A 分量 8 次全漏，Vdc 分量全部检出），延迟中位 2 周期，注入前误报 0 / 416 周期，撤除后 8 / 1248（全部 MPCC_P）；与离线参考（features.py + 位精确 ONNX）检出数相同，3640 个公共周期中 35 个标志字不同。（3）估计器：HGQ2 变种基线 THD50 3.18% / 3.02%（FFT 2.76%），交流链偏置下最好（E-AC-01b 5.97% / 6.42% 对 7.78%），直流/电池链偏置下差约 0.5 pp，恢复时间与 F1 同级；详见 `EMI_DET_FPGA/README.md` 评估战役一节与 `runs/estimator_report/`。（4）`artifacts/detector.onnx` 已换成位精确版本；`make_injection_figs`、`make_waveform_figs` 扩到 8 种策略并重绘。 |
