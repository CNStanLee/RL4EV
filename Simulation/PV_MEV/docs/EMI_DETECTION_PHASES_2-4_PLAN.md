...
| 2026-09-04 06:20–09:00 | det_Iref 改接 Speed Regulator2 后快照重做，评估战役重启（07:00，3 进程，预计 17:30 完成）；上板组件与 ONNX 位精确修正 | （1）前 3 次运行的 SIL 对照：43 个特征中除占空比两项外与 Python 参考一致到 1e-13，ONNX 输出一致到 5e-14；CRPR 的 iac_ref_corr 差异只在第一个周期（日志首样本 NaN），不是错误。发现 SIL 特征块的占空比 `d_mean`、`d_ff_mismatch` 比离线特征滞后整整一个周期（20 ms），其余信号不滞后；战役不重启，跑完后离线量化其影响并在板级特征块中修正。（2）上板：`scripts/make_board_components.py` 生成 `HLS_PRJ/emi_detector`、`HLS_PRJ/harmonic_estimator` 两个 Vitis 统一流程组件（与 `HLS_PRJ/mpcc` 同款 `hls_config.cfg` / `vitis-comp.json` / s_axilite），包装层分别负责标准化+阈值、CycleNorm+解码；本机无 Vitis，用 zig c++ 做 C 仿真：估计器与 ONNX 参考逐值相同，检测器 max |Δlogit| 4.6e-5、1089 周期标志字全部一致。（3）发现 Keras 导出的 detector.onnx 用四舍六入五成双而硬件逢五进一，8709 周期 max |Δlogit| 4.3、68 个周期判定不同；`export_bitexact_onnx.py` 生成的 `detector_bitexact.onnx` 与 Keras 逐值相同（max 0），战役结束后替换并用 `redecide.py` 复核（前 6 次运行复核：判定无差异）。 |
