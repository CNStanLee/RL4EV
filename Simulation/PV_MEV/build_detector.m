...
set_mlfcn([d '/emi_features'], features_code(), {'X', '[200 12]'; 'f', '[1 48]'});
add_block('simulink/Discrete/Unit Delay', [d '/ema'], 'SampleTime', '0.02', 'InitialCondition', 'zeros(1,49)', 'Position', [420 300 460 330]);
...
set_mlfcn([d '/emi_augment'], augment_code(), {'f', '[1 48]'; 'st', '[1 49]'; 'onehot', '[1 6]'; 'alpha', '1'; 'x', '[1 48]'; 'stn', '[1 49]'});
...
onnx_file = fullfile(fileparts(fileparts(mdir)), 'EMI_DET_FPGA', 'artifacts', 'detector.onnx');   % literal path (System block text params are not evaluated); OnnxRunner.resolve remaps it on another checkout
set_param([d '/ONNX Runner'], 'ModelFile', strrep(onnx_file, '\', '/'), 'NumIn', '48', 'NumOut', '20', 'SampleTime', '0.02');   % v5: 48 features -> [5 logits, 10 zeros, 5 amplitudes]
...
add_block('simulink/Sources/Constant', [d '/hyst'], 'Value', 'det_hyst', 'Position', [800 440 880 460]);
add_block('simulink/Discrete/Unit Delay', [d '/flag'], 'SampleTime', '0.02', 'InitialCondition', 'zeros(1,5)', 'Position', [800 480 840 510]);
...
set_mlfcn([d '/emi_decide'], decide_code(), {'raw', '[1 20]'; 'thr', '[1 5]'; 'persist', '1'; 'cnt', '[1 5]'; 'hyst', '1'; 'flag', '[1 5]'; 'chan', '[1 5]'; 'cls', '1'; 'conf', '1'; 'amp', '[1 5]'; 'cntn', '[1 5]'; 'flagn', '[1 5]'});
...
add_line(d, 'hyst/1', 'emi_decide/5'); add_line(d, 'flag/1', 'emi_decide/6');
add_line(d, 'emi_decide/5', 'cnt/1'); add_line(d, 'emi_decide/6', 'flag/1');
% ---- outputs
...
'end\n' ...
'prev = row;\n' ...
'%% v3 extras (features.FEATURE_NAMES_V3_EXTRA): bus 50 Hz ripple, its ratio to the 100 Hz ripple, its phase vs Vac, duty half-cycle asymmetry, pad\n' ...
'vh1 = 2/n*hypot(vdc''*c1, vdc''*s1); vh2 = 2/n*hypot(vdc''*c2, vdc''*s2);\n' ...
'vph = angle(exp(1i*(atan2(-(vdc''*s1), vdc''*c1) - ph_v)));\n' ...
'sp = sin(th) > 0; sn = sin(th) < 0; dp = 0; dn = 0; if any(sp), dp = mean(d(sp)); end; if any(sn), dn = mean(d(sn)); end\n' ...
'f = [row, dl, vh1, vh1/max(vh2, 0.1), vph, dp - dn, 0];\n']);
end
...
'%%#codegen\n' ...
'%% st = [ema(1:48), n_cycles_seen]\n' ...
'ema = st(1:48); cnt = st(49);\n' ...
'warm = 3;\n' ...
'if cnt < warm\n' ...
...
'end\n' ...
'stn = [ema, cnt + 1];\n' ...
'x = f;   %% 48 base features only (baseline-relative block and one-hot did not help); dev / onehot kept for later\n']);
end

function s = decide_code()
s = sprintf(['function [chan, cls, conf, amp, cntn, flagn] = emi_decide(raw, thr, persist, cnt, hyst, flag)\n' ...
'%%#codegen\n' ...
'%% raw = [5 channel logits, 10 class logits (unused, zeros), 5 signed normalized amplitudes]\n' ...
'%% set a channel after `persist` consecutive cycles >= thr, clear it when p < hyst*thr (hysteresis)\n' ...
'pc = 1 ./ (1 + exp(-double(raw(1:5))));\n' ...
'hit = double(pc >= thr); low = double(pc < hyst .* thr);\n' ...
'cntn = (cnt + 1) .* hit;                 %% consecutive cycles above threshold\n' ...
'flagn = flag .* (1 - low) + (1 - flag) .* double(cntn >= persist);\n' ...
'chan = flagn;\n' ...
'z = double(raw(6:15)); z = z - max(z); p = exp(z) / sum(exp(z));\n' ...
'[conf, i] = max(p); cls = i - 1;         %% 0 none .. 9 multi (features.CLASSES order)\n' ...
'amp = double(raw(16:20)) .* chan;        %% per-channel amplitude (Vdc/100 V, Vac/40 V, Iac/20 A, Vbat/25 V, Ibat/8 A), 0 while not flagged\n']);
end
