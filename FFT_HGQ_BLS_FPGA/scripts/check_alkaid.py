#!/usr/bin/env python3
"""Trace a trained HGQ2 model to ALIR and require bit-exact parity."""
...
def main() -> None:
    ...
    from hgq_model.hardware import check_alkaid_conversion
    ...
    manifest = check_alkaid_conversion(
        args.model,
        model_path,
        dataset_path,
        output_dir,
    ...
    print(
        f"converted {args.model}: {output_dir} "
        f"(ALIR exact={manifest['parity']['exact']}, bytes={manifest['alir_bytes']})"
    )
